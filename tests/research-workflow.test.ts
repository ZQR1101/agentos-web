...
function dependencies(overrides: Partial<ResearchWorkflowDependencies> = {}): ResearchWorkflowDependencies {
  return {
    ...
    plan: async () => ({ plan: { searchQuery: "agent governance", subquestions: ["question one", "question two", "question three"], successCriteria: ["criterion one", "criterion two", "criterion three"] }, invocation: { agent: "Planner", model: "fake", latencyMs: 10, totalTokens: 100 } }),
    ...
    write: async () => ({ report: `[来源 1](${source.url})`, responseId: "fake-response", rewrittenCitationCount: 0, removedExternalLinkCount: 0, invocation: { agent: "Executor", model: "fake", latencyMs: 20, totalTokens: 200 } }),
    review: async () => ({ approved: true, score: 90, issues: [], revisionInstructions: "", citationCheck: { valid: true, issues: [], citationCount: 1 }, invocation: { agent: "Reviewer", model: "fake", latencyMs: 30, totalTokens: 300 } }),
    ...overrides,
  };
}
...
test("Background workflow completes with injected model and MCP dependencies", async () => {
  await inTemporaryStore(async () => {
    ...
    assert.equal(completed?.observability?.modelCalls.length, 3);
    assert.equal(completed?.observability?.totalTokens, 600);
  });
});
...
test("Cancelled workflow stops after the in-flight stage and does not call search", async () => {
  await inTemporaryStore(async () => {
    ...
    const run = runResearchWorkflow(task.id, executionId, dependencies({
      plan: async () => new Promise((resolve) => { releasePlan = () => resolve({ plan: { searchQuery: "agent governance", subquestions: ["question one", "question two", "question three"], successCriteria: ["criterion one", "criterion two", "criterion three"] } }); }),
