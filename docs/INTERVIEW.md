...
外部搜索和模型调用前需要人工审批，Research API 必须收到明确的 `externalTools: true`，不能只靠前端按钮状态。MCP 调用还要经过默认拒绝的 Tool Policy：工具必须已注册、启用、作用域匹配且满足审批策略。每次调用随后通过 Harness 预算授权，授权动作和当前用量会先写入 Task Store。DeepSeek 单次调用限制 45 秒，Tavily 限制 20 秒。任务支持暂停、恢复、失败重试和协作式取消：取消会阻止当前外部调用完成后的所有后续步骤。密钥只存在服务端。网页摘要统一视为不可信数据，Source Policy 会进行 URL 校验、质量评分和提示注入检测，高风险来源不会进入模型上下文。
