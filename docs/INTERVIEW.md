...
Task Store 将状态检查与 `waiting_approval → running` 更新放在同一个串行临界区，只有一个请求能获得带 `executionId` 的执行权。Research API 在领取执行权后立即返回 202，由后台 Worker 执行；其他请求复用同一个运行中 Task，工作台通过 SSE 接收快照，因此不会重复调用 DeepSeek 或 Tavily。并发测试会用 20 个请求竞争同一个任务，断言只有一个成功 claim。

### 如何保证可控？

外部搜索和模型调用前需要人工审批，Research API 必须收到明确的 `externalTools: true`，不能只靠前端按钮状态。MCP 调用还要经过默认拒绝的 Tool Policy：工具必须已注册、启用、作用域匹配且满足审批策略。每次调用随后通过 Harness 预算授权，授权动作和当前用量会先写入 Task Store。每个模型调用还会记录 Agent 身份、耗时、Token 用量与响应 ID；若部署环境配置了每百万 Token 单价，系统才计算估算成本，避免将不稳定价目表硬编码为真实账单。DeepSeek 单次调用限制 45 秒，Tavily 限制 20 秒。任务支持暂停、恢复、失败重试和协作式取消：取消会阻止当前外部调用完成后的所有后续步骤。密钥只存在服务端。网页摘要统一视为不可信数据，Source Policy 会进行 URL 校验、质量评分和提示注入检测，高风险来源不会进入模型上下文。

### 如何降低单一厂商来源带来的偏差？

Source Policy 在安全筛选后按质量排序，并限制同一域名最多保留 2 个来源。它还会标注来源类型、可信度和可得的发布时间；任务完成时再结算来源广度、类型多样性、高可信来源、近期来源和有效引用覆盖。发布时间缺失会明确标为未知，证据覆盖分数只衡量来源组合的广度与元数据，不伪装成逐句事实核验。Harness 会分别记录安全隔离、URL 去重、域名多样性排除和数量截断，避免把“未采用”笼统描述成风险来源。
...
Writer 不再拥有输出最终 URL 的权限：它只能写 `[来源 N]`，服务端再从通过 Source Policy 的来源表生成 canonical URL。即使模型沿用旧格式并给出错误链接，渲染器也会按编号改写为正确白名单 URL；其他 Markdown 外链和裸 URL 会被移除，并记录到 Harness 事件。这样既不放松引用校验，也避免无意义的格式性失败。
...
本地 JSON 存储与单进程 Worker 只适合单机 Demo。项目已有后台执行、SSE、幂等执行与协作式取消；生产化仍需要 PostgreSQL、Redis 任务队列、分布式锁和可把取消信号传到第三方请求的 Worker。
...
- 实现任务持久化与运行记录，支持刷新后恢复任务；实现后台 Worker、SSE 快照与协作式取消，并为 Redis/PostgreSQL 迁移预留清晰边界。
